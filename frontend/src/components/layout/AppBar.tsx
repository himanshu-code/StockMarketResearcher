  ...
  return (
    <AppBar position="fixed" elevation={0}>
      <Toolbar className="max-w-[1440px] mx-auto w-full px-4 sm:px-6">
        {/* Brand */}
        <Box className="flex items-center gap-2 cursor-pointer" onClick={() => router.push('/')}>
          <LogoIcon />
          <Typography
            variant="body1"
        ...
        <Box className="flex items-center gap-1">
          {navLinks.map(link => (
            <Button
              ...
              size="small"
              sx={{
                color: pathname === link.path ? '#E1E2EB' : '#BBCBB2',
                fontWeight: pathname === link.path ? 600 : 400,
                borderBottom:
                  pathname === link.path ? '2px solid #00C805' : '2px solid transparent',
                borderRadius: 0,
                px: 1.5,
                py: 0.75,
